...
public class CrudFile implements CRUD<String, Integer> {
    ...
    public CrudFile(String crudFilePath) {
        Objects.requireNonNull(crudFilePath, "path cannot be null");
        this.crudFile = new File(crudFilePath);
        this.size = this.CountLines();
        if (!this.crudFile.exists()) {
    ...
    public Integer create(String entity, File path) throws IOException{
        if (!copied) {
            monitoredFile = path;
            copyFile(monitoredFile, crudFile);
            copied = true;
